...
- **DTO/контракты**:  
- Убрать из `PostCreateDto`/`PostCreateRequest` поле `authorId` — бэкенд уже берёт автора из `UserPrincipal`. Это снизит риск подмены автора.  
...
- Явно зафиксировать в `SecurityConfig` правило `requestMatchers("/api/admin/**").hasRole("ADMIN")` (сейчас ограничение обеспечивается аннотацией `@PreAuthorize` в контроллере, но дублирование на уровне http-матчинга упростит чтение).
